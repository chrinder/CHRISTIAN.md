...
- In namespaced scratch orgs, `apex://` action targets must use the namespace prefix at publish-time (e.g. `apex://my_ns__ClassName`).
- Deploy from repository paths only (`force-app/...`). Avoid temporary copy paths — local source tracking can become corrupted with unsafe filepaths.
