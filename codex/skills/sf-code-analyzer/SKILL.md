...
description: Run Salesforce Code Analyzer on Apex, Trigger, Flow, or metadata files. Use after writing or modifying .cls, .trigger, .xml, or flow files, or when asked to scan, lint, or check code quality.
metadata:
  short-description: Run Salesforce code analysis
---
...
- If the user provides a file or directory path, scan that path
...
- User asks for "security" -> security scan only
- User asks for "cleancode" -> clean code scan only
...
- Do not auto-fix without asking, just report
